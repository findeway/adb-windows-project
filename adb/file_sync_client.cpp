...
{
	//将rpath转换成utf8格式
	std::string utf8_rPath;
	GBK_to_UTF8(rpath,strlen(rpath),utf8_rPath);

	//将lpath转换成utf8格式
	std::string utf8_lPath;
	GBK_to_UTF8(lpath,strlen(lpath),utf8_lPath);
    ...
    if(writex(fd, &msg.req, sizeof(msg.req)) ||
       writex(fd, utf8_rPath.c_str(), len) || writex(fd, tmp, r)) {
        free(file_buffer);
        goto fail;
    }
   ...
   // while((de = readdir(d))) 
	do 
	{
...
#ifdef _WIN32  
		std::string utf8name;  
		GBK_to_UTF8(name,strlen(name),utf8name);
#endif  
        /*
        ...
        if (S_IFDIR&st.st_mode) {
            ci = mkcopyinfo(lpath, rpath, utf8name.c_str(), 1);
            ci->next = dirlist;
            dirlist = ci;
        } else {
